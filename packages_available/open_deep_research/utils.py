...
##########################
# Serp Search Tool Utils
##########################
SERP_SEARCH_DESCRIPTION = (
    "A search engine optimized for comprehensive, accurate, and trusted results. "
    "Useful for omics research, scientific literature, and current events."
)
@tool(description=SERP_SEARCH_DESCRIPTION)
def serp_search(query: str, max_results: int = 5) -> str:
    """Search using Google Scholar via SerpAPI for scientific literature."""
    
    serp_api_key = os.getenv("SERP_API_KEY")
    if not serp_api_key:
        return "SERP_API_KEY not found in environment."
    ...
    for i, result in enumerate(results[:max_results], 1):
        formatted_output += f"--- SOURCE {i}: {result.get('title', 'No title')} ---\n"
        ...
        if authors:
            ...
            formatted_output += f"Authors: {author_names}\n"
...
async def get_search_tool(search_api: SearchAPI):
    ...
    Args:
        search_api: The search API provider to use (Serp, Anthropic, OpenAI, or None)
        
    Returns:
        List of configured search tool objects for the specified provider
    """
    if search_api == SearchAPI.SERP:
