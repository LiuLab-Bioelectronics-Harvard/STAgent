...
def nn_approx(ds1, ds2, names1, names2, knn=50):
    # hnswlib is sensitive to dtype/contiguity and query params.
    # Ensure inputs are contiguous float32, clamp k, and keep ef >= k to avoid crashes.
    ds2 = np.asarray(ds2, dtype=np.float32, order='C')
    ds1 = np.asarray(ds1, dtype=np.float32, order='C')

    dim = ds2.shape[1]
    num_elements = ds2.shape[0]
    k = int(min(knn, num_elements))
    if k <= 0:
        return set()

    p = hnswlib.Index(space='l2', dim=dim)
    # ef_construction/ef should be >= k; use modest buffers for stability
    p.init_index(max_elements=num_elements, ef_construction=max(100, k), M=16)
    p.set_ef(max(10, k))
    p.add_items(ds2)
    ind, distances = p.knn_query(ds1, k=k)
    match = set()
