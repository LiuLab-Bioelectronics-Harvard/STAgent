...
def Cal_Spatial_Net(adata, rad_cutoff=None, k_cutoff=None,
    ...
    #########
    # adata.X may be sparse (has .toarray()) or already a dense numpy.ndarray.
    # Handle both to avoid: AttributeError: 'numpy.ndarray' object has no attribute 'toarray'
    X_mat = adata.X.toarray() if sp.issparse(adata.X) else np.asarray(adata.X)
    X = pd.DataFrame(X_mat[:, ], index=adata.obs.index, columns=adata.var.index)
